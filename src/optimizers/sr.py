    ...
    def step(self, params, grads, sr_matrices=None):
        ...
        if sr_matrices is not None:
            for key, sr_matrix in sr_matrices.items():
                ...
                dgd = np.linalg.norm(condit_grad.T @ grads[key]) # this is dtheta^T * g dtheta # TODO: check this dim

                self.trust_regions[key] = np.min([self.delta0, np.sqrt(self.delta_1/dgd)])
                grads[key] = condit_grad.reshape(params.get(key).shape) # sheng cals this Gj


        # Fj is what sheng calls the original grad without the sr matrix
        for key, grad in grads.items():
            params.set([key], [params.get(key) - self.trust_regions[key] * grad/np.sqrt(self.t)])
